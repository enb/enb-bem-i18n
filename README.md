...
С помощью ехнологий пакета `enb-bem-i18n` осуществляется сборка модуля для интернационализации (`i18n`).

**Технологии пакета `enb-bem-i18n`:**

* [keysets](/api.ru.md#keysets) — служебная технология для сборки исходных файлов с переводами.
* [i18n-js](/api.ru.md#i18n-js) — технология для формирования бандлов с переводами для каждого языка, которые будут использоваться в дальнейшем.
...
Ядро `i18n` — это библиотека для интернационализации. Ядро находится в keysets-файлах (`<block-name>.i18n.js` или `all.js`) в одной из базовых библиотек блоков.
...
* В `bem-bl` переводы, одинаковые для всех языков, хранятся в файле `all.js`.
